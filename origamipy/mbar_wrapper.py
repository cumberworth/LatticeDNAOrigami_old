...
class MBARWrapper:
    ...
    def calc_all_expectations(self, filebase):
        ...
        all_aves = []
        all_stds = []
        condition_tags = self._decor_outs.all_conditions.condition_tags
        series_tags = self._decor_outs.all_series_tags
        for i, se_tag in enumerate(series_tags):
            print('Calculating expectation of {} ({} of {})'.format(se_tag, i,
                len(series_tags)))
            values = self._decor_outs.get_concatenated_series(se_tag)
            aves, stds = self._calc_expectations(values)
            condition_tags.append(se_tag)
        ...
        aves_file.write(condition_tags, np.concatenate(
            [all_conds, np.array(all_aves).T], axis=1))
        stds_file = files.TagOutFile('{}.stds'.format(filebase))
        stds_file.write(condition_tags, np.concatenate(
            [all_conds, np.array(all_stds).T], axis=1))
    ...
    def calc_1d_lfes(self, reduced_conditions, filebase, xtag='temp'):
        ...
        condition_tags = reduced_conditions.condition_tags
        xvar_i = condition_tags.index(xtag)
        xvars = [c[xvar_i] for c in all_conds]
        series_tags = self._decor_outs.all_series_tags

        # Also bad
        for se_tag in ['tenergy', 'henthalpy', 'hentropy', 'stacking', 'bias']:
            series_tags.remove(se_tag)

        for i, se_tag in enumerate(series_tags):
            print('Calculating 1D LFEs of {} ({} of {})'.format(se_tag, i, len(series_tags)))
            values = self._decor_outs.get_concatenated_series(se_tag)
            ...
            lfes_filebase = '{}_{}-lfes'.format(filebase, se_tag)
    ...
    def calc_specified_2d_lfes(self, se_tag_pairs, reduced_conditions, filebase):
        print('Calculating 2D LFEs for all specified pairs')
        for i, se_tag_pair in enumerate(tag_pairs):
            se_tag1, se_tag2 = se_tag_pair
            print('Calculating 2D LFEs of {}-{} ({} of {})'.format(se_tag1, se_tag2, i, len(se_tag_pairs)))
            self.calc_2d_lfes(se_tag1, se_tag2, reduced_conditions, filebase)

    def calc_2d_lfes(self, se_tag1, se_tag2, reduced_conditions, filebase):
        print('Calculating 2D LFEs of {}-{}'.format(se_tag1, se_tag2))
        temps = [c.temp for c in reduced_conditions]
        decor_value_pairs = list(zip(self._decor_outs.get_concatenated_series(se_tag1),
                                 self._decor_outs.get_concatenated_series(se_tag2)))
        bins = list(set(decor_value_pairs))
        lfes, stds = self._calc_lfes(bins, decor_value_pairs, reduced_conditions)

        # Ugly
        header = np.concatenate([[se_tag1, se_tag2], temps])
        lfes_filebase = '{}_{}-{}-lfes'.format(filebase, se_tag1, se_tag2)
