...
void enumerate_main(
    ...
    if (params.m_enumerate_staples_only) {
        conf_enumerator = new StapleConformationalEnumerator {
                origami, ops, biases, params.m_ops_to_output};
    }
    else {
...
double ConformationalEnumerator::calc_multiplier(
        Domain* domain,
        Domain* other_domain) {
...
void StapleConformationalEnumerator::grow_next_domain(
    ...
    if (not m_domains.empty()) {
        ...
        if (growth_off_scaffold) {
            ...
            m_energy += m_origami_system.set_domain_config(
                    *next_domain, p_new, o_new);
            if (m_origami_system.m_constraints_violated == true) {
                m_origami_system.m_constraints_violated = false;
...
void MisbindingGrowthpointEnumerator::enumerate() {
    // Recursively enumerate all configurations for given scaffold and staple
    // set
...
void MisbindingGrowthpointEnumerator::enumerate_internal() {
    ...
    for (size_t i {0}; i != m_staples.size(); i++) {
        ...
        for (size_t j {0}; j != m_unbound_system_domains.size(); j++) {
            ...
            m_unbound_system_domains.erase(
                    m_unbound_system_domains.begin() + j);
...
void NoMisbindingGrowthpointEnumerator::enumerate_internal() {
    ...
    for (size_t i {0}; i != m_staples.size(); i++) {
        ...
        // Iterate through available domains for growthpoint
        for (size_t j {0};
             j != m_unbound_system_domains[staple_ident - 1].size();
             j++) {
